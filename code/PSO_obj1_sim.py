...
def objective_function(particle):
    ...
    for t in range(D):
        prefactor = (D - t) / (D - 1)  # Prefactor decreases linearly over time
        N_t = np.sum(particle[:, t])  # Sum current for all EVs at time t
        total_quick_charge += prefactor * N_t  # Incentivize early charging
    return total_quick_charge
...
plt.tight_layout()
plt.savefig('PSO_obj1.jpg', dpi = 600)
#plt.show()
