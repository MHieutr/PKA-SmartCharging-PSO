...
def objective_function(particle):
    column_sum = particle.sum(axis=0)  # Tổng công suất tại mỗi thời điểm
    target_range = 0.05 * max_power_limit  # Biên độ an toàn ±5%

    # Thưởng cao nếu tổng công suất nằm gần max_power_limit
    reward = np.sum(-np.abs(column_sum - max_power_limit) + max_power_limit)
    # Phạt nếu vượt quá mức giới hạn
...
plt.tight_layout()
plt.savefig('GA_obj5.jpg', dpi = 600)
#plt.show()