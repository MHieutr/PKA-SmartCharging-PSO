...
w_QC = 0.3  # Weight for Quick Charging
w_NC = 0.3  # Weight for Non-Completion Penalty
w_LV = 0.3  # Weight for Load Variations
# Utils
...
def objective_function_qc(particle):
    quick_charge_reward = 0
    for k in range(D):
        time_factor = (D - k) / D  # Reduces reward over time
        quick_charge_reward += time_factor * np.sum(particle[:, k] * U)
    return quick_charge_reward

# Objective Function 2: Non-Completion Penalty
def objective_function_nc(particle, alpha=1):
    penalty = 0
    for i in range(N):
...
# Objective Function 3: Load Balance
def objective_function_lv(particle):
    load_variation = 0
    for t in range(D):
        N_t = np.sum(particle[:, t])
        load_variation += -N_t ** 2  # We aim to maximize negative variation to flatten power usage
    return load_variation

# Composite Objective Function
def objective_function(particle):
    qc = objective_function_qc(particle)
    nc = objective_function_nc(particle)
    lb = objective_function_lv(particle)
    return w_QC * qc + w_NC * nc + w_LV * lb
...
for sim in range(nSim):
    ...
    for iteration in range(max_iterations):
        for i in range(num_particles):
            ...
            if value > objective_function(personal_best_positions[i]):
...
plt.tight_layout()
plt.savefig('PSO_obj4.jpg', dpi = 600)
#plt.show()
